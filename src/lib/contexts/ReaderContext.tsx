...
export function ReaderStateProvider({ children }: { children: ReactNode }) {
  ...
  const setDisplayMode = useCallback(
    (displayMode: DisplayMode) => updateDisplay({ displayMode }),
    [updateDisplay]
  );
  const setShowTranslit = useCallback(
    (v: boolean) => updateDisplay({ showTranslit: v }),
    [updateDisplay]
  );
  const setShowParallel = useCallback(
    (v: boolean) => updateDisplay({ showParallel: v }),
    [updateDisplay]
  );
  const setMaskKnown = useCallback(
    (v: boolean) => updateDisplay({ maskKnown: v }),
    [updateDisplay]
  );
  const setInterlinearMode = useCallback(
    (v: boolean) => updateDisplay({ interlinearMode: v }),
    [updateDisplay]
  );
  const setFontSize = useCallback(
    (size: number) => updateDisplay({ fontSize: size }),
    [updateDisplay]
  );
  const setHighlightIntensity = useCallback(
    (v: HighlightIntensity) => updateDisplay({ highlightIntensity: v }),
    [updateDisplay]
  );
  const setSwipeMovesToNext = useCallback(
    (v: boolean) => updateDisplay({ swipeMovesToNext: v }),
    [updateDisplay]
  );
  const resetDisplay = useCallback(
    () => setState((prev) => ({ ...prev, display: DEFAULT_READER_STATE.display })),
    []
  );

  // ── Navigation actions ──
  const setTextId = useCallback(
    (id: string | null) =>
      updateNavigation({
        textId: id,
  ...
  const setSentenceIndex = useCallback(
    (index: number) => updateNavigation({ currentSentenceIndex: index }),
    [updateNavigation]
  );
  const setScrollPage = useCallback(
    (page: number) => updateNavigation({ currentScrollPage: page }),
    [updateNavigation]
  );
  const setScrollProgress = useCallback(
    (progress: number) => updateNavigation({ scrollProgress: progress }),
    [updateNavigation]
  );
      ...
      if (next >= 0) {
        return {
          ...
          navigation: {
            ...prev.navigation,
            currentChapterIndex: next,
            currentSentenceIndex: prevChapterSentences - 1,
            currentScrollPage: 0,
          },
        };
      }
  ...
  const resetNavigation = useCallback(
    () => setState((prev) => ({ ...prev, navigation: DEFAULT_READER_STATE.navigation })),
    []
  );
  ...
  const togglePlay = useCallback(
    () =>
      setState((prev) => ({ ...prev, audio: { ...prev.audio, isPlaying: !prev.audio.isPlaying } })),
    []
  );
  const setPlayState = useCallback(
    (playing: boolean) =>
      setState((prev) => ({ ...prev, audio: { ...prev.audio, isPlaying: playing } })),
    []
  );
  ...
  const toggleLoopSentence = useCallback(
    () =>
      setState((prev) => ({
        ...prev,
        audio: { ...prev.audio, loopSentence: !prev.audio.loopSentence },
      })),
    []
  );
  const toggleLoopWord = useCallback(
    () =>
      setState((prev) => ({ ...prev, audio: { ...prev.audio, loopWord: !prev.audio.loopWord } })),
    []
  );
  const setAudioPosition = useCallback(
    (sentenceIdx: number, wordIdx: number) => updateAudio({ position: { sentenceIdx, wordIdx } }),
    [updateAudio]
  );
  ...
  const advanceAudioWord = useCallback((maxWords: number) => {
    setState((prev) => {
      ...
      if (nextWord < maxWords) {
        return {
          ...prev,
          audio: { ...prev.audio, position: { ...prev.audio.position, wordIdx: nextWord } },
        };
      }
      return prev;
    });
  ...
  const advanceAudioSentence = useCallback((maxSentences: number) => {
    setState((prev) => {
      ...
      if (nextSentence < maxSentences) {
        return {
          ...prev,
          audio: { ...prev.audio, position: { sentenceIdx: nextSentence, wordIdx: 0 } },
        };
      }
      return {
        ...prev,
        audio: { ...prev.audio, isPlaying: false, position: { sentenceIdx: 0, wordIdx: 0 } },
      };
    });
  }, []);

  const resetAudio = useCallback(
    () => setState((prev) => ({ ...prev, audio: DEFAULT_READER_STATE.audio })),
    []
  );
