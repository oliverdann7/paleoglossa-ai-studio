...
function getCorpusTokens(text: any) {
  ...
  const sections =
    text.sectionsPreview?.map((p: any) => CorpusDB.getSection(p.id)).filter(Boolean) || [];
...
const DEFAULT_TEXT_METADATA: Record<string, Partial<LibraryText>> = {
  ...
  'Egy-Ptah-1': {
    date: 'Middle Kingdom tradition',
    period: 'Middle Egyptian',
    genre: 'Wisdom instruction',
    corpusType: 'classical',
  },
  'Anabasis-1': {
    date: '4th c. BCE',
...
function inferMockMetadata(text: any): Partial<LibraryText> {
  ...
  if (text.language === 'grc-koine')
    return {
      date: '1st-2nd c. CE',
      period: 'Hellenistic / Roman',
      genre: title.includes('gospel') ? 'Gospel' : 'Early Christian',
      corpusType:
        title.includes('1 clement') || title.includes('didache') ? 'patristic' : 'biblical',
    };
  if (text.language === 'hbo')
    return {
      date: '1st millennium BCE',
      period: 'Iron Age',
      genre: 'Biblical prose/poetry',
      corpusType: 'biblical',
    };
  if (text.language === 'lat')
    return {
      date: '1st c. BCE-1st c. CE',
      period: 'Classical / Imperial',
      genre: title.includes('annals') ? 'History' : 'Classical literature',
      corpusType: 'classical',
    };
  if (text.language === 'syr' || text.language === 'cop')
    return {
      date: 'Late Antique',
      period: 'Late Antique',
  ...
  if (text.language === 'arc')
    return {
      date: 'Late Antique',
      period: 'Late Antique',
      genre: title.includes('papyri') ? 'Documentary text' : 'Targum',
      corpusType: title.includes('papyri') ? 'manuscript' : 'biblical',
    };
  if (text.language === 'akk' || text.language === 'hit' || text.language === 'egy')
    return {
      date: '2nd-1st millennium BCE',
      period: 'Ancient Near Eastern',
      genre: title.includes('letter') ? 'Letter' : 'Royal / literary text',
      corpusType: title.includes('letter') || title.includes('laws') ? 'inscription' : 'classical',
    };
  if (text.language === 'san')
    return {
      date: '1st millennium BCE-1st millennium CE',
      period: 'Classical Sanskrit',
...
function getTextMetadata(text: any): Partial<LibraryText> {
  return {
    ...
    corpusType:
      text.corpusType ||
      DEFAULT_TEXT_METADATA[text.id]?.corpusType ||
      inferMockMetadata(text).corpusType,
  };
}
...
export class LibraryService {
  ...
  static async getRawTexts(
    ...
    if ((!source || source === 'import') && userId) {
      ...
      userImports.forEach((t) => {
        const hasAiMorphology =
          t.analysisStatus === 'analyzed' ||
    ...
    rawTexts.forEach((tx) => {
      ...
      if (tx.sourceType === 'corpus') {
        ...
        allTokens.forEach((tok: any) => {
          ...
          if (info.state === 'KNOWN') {
            knownCount++;
            if (tok.lemma) knownUniqueLemmas.add(tok.lemma);
          } else if (info.state !== 'NEW') learningCount++;
        });
      } else if (tx.sourceType === 'import') {
        const t = tx.rawTextReference as ImportedText;
        const tokens =
          t.sentences?.flatMap((s: any) => s.tokens).filter((tok: any) => tok.type === 'word') ||
          [];
        if (tokens.length > 0) {
      ...
      const percentKnownUnique =
        uniqueTotal > 0 ? Math.round((knownUniqueLemmas.size / uniqueTotal) * 100) : 0;
      const difficultyTier: CoverageResult['difficultyTier'] =
        percentKnownUnique >= 95
          ? 'beginner'
          : percentKnownUnique >= 85
            ? 'accessible'
            : percentKnownUnique >= 70
              ? 'challenging'
              : 'advanced';
  ...
  static async getLibrary(
    ...
    const coverageMap = getWordInfo
      ? this.computeCoverage(rawTexts, getWordInfo)
      : new Map<string, CoverageResult>();
