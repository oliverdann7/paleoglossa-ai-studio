...
import { lookupLexicon, type LexiconLookupResult } from '@/lib/services/lexiconService';
...
} from '@/lib/data/dictionarySources';
import { resolveMeaning, type ResolvedMeaning } from '@/lib/services/dictionaryResolver';
...
export const LexDrawerPanel = memo(
  ({
    ...
    // ── Dictionary source picker (LingQ-style, per-language) ──────────────────
    // Only sources valid for THIS language are ever offered — separation is
    // guaranteed by getDictionariesForLanguage.
    const dictionarySources = useMemo(() => getDictionariesForLanguage(langId), [langId]);
    const inlineDictionaries = useMemo(
      () => dictionarySources.filter((s) => s.resolver !== 'external-link'),
      [dictionarySources]
    );
    const [selectedSourceId, setSelectedSourceId] = useState(() =>
      resolvePreferredDictionaryId(langId, settings.preferredDictionaryByLang)
    );
    const [sourceMeaning, setSourceMeaning] = useState<ResolvedMeaning | null>(null);
    const [isSourceMeaningLoading, setIsSourceMeaningLoading] = useState(false);

    // Re-seed the selected source from the saved preference when the language
    // changes (e.g. switching texts).
    useEffect(() => {
      setSelectedSourceId(resolvePreferredDictionaryId(langId, settings.preferredDictionaryByLang)); // eslint-disable-line react-hooks/set-state-in-effect
    }, [langId, settings.preferredDictionaryByLang]);

    const handleSelectSource = (sourceId: string) => {
      setSelectedSourceId(sourceId);
      // Persist as this language's default for next time.
      void updateSettings({
    ...
    const handleFetchAIMorphology = async () => {
      ...
      try {
        const result = await AIClient.getAIMorphology(
          langId,
          selectedWord.text,
          selectedWord.lemma
        );
    ...
    const definitionLookup = useMemo(() => {
      ...
      if (selectedWord.gloss) {
        return {
          definition: selectedWord.gloss,
          source: GLOSS_SOURCES.TOKEN_GLOSS,
        } as LookupResult;
      }
      return null;
    }, [selectedWord, wordInfo?.userGloss, langId]);

    // Resolve the meaning from the user's chosen dictionary source. AI sources
    // only fire when explicitly selected (default is the offline bundled lexicon).
    useEffect(() => {
      if (!selectedWord) return;
      const src = getDictionarySource(selectedSourceId);
      setSourceMeaning(null); // eslint-disable-line react-hooks/set-state-in-effect
      if (!src || src.resolver === 'external-link') {
        setIsSourceMeaningLoading(false);
        return;
      }
      let cancelled = false;
      setIsSourceMeaningLoading(true);
      (async () => {
    ...
    // localized hook below renders it in the UI language.
    const baseMeaning = useMemo(() => {
      if (!selectedWord) return null;
      if (wordInfo?.userGloss) {
        return {
          text: wordInfo.userGloss,
          label: t('reader.yourTranslation', 'Your translation'),
          isUserGloss: true,
        };
      }
      if (sourceMeaning?.definition) {
        return {
          text: sourceMeaning.definition,
      ...
      if (definitionLookup?.definition && definitionLookup.source !== GLOSS_SOURCES.USER_GLOSS) {
        return { text: definitionLookup.definition, label: definitionLookup.sourceLabel };
      }
      if (lexiconResult?.definition) {
        return {
          text: lexiconResult.definition,
          label: lexiconResult.sourceLabel,
          url: lexiconResult.sourceUrl,
        };
      }
      if (wiktionaryResult) {
        const headline = summarizeWiktionary(wiktionaryResult);
        if (headline) {
          return { text: headline, label: 'Wiktionary', url: wiktionaryResult.sourceUrl };
        }
      }
      if (aiFallbackGloss) {
    ...
    const handleSaveAsTranslation = () => {
      ...
      updateGloss(selectedWord.lemma, displayMeaningText, langId);
      setGlossSaved(true);
      trackEvent(ANALYTICS_EVENTS.WORD_GLOSS_SAVED, {
        languageId: langId,
        lemmaLength: selectedWord.lemma?.length || 0,
        glossLength: displayMeaningText.length,
        textId: text?.id,
      });
      ...
      <AnimatePresence>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-8 pb-safe">
                ...
                const visibleDicts = entry?.dictionaries
                  ? entry.dictionaries.filter(
                      (d) =>
                        ...
                        settings.activeDictionaries.length === 0 ||
                        settings.activeDictionaries.includes(d.id)
                    )
                  : [];
                return (
                  <div className="mb-6 flex flex-wrap items-center gap-2 text-[11px] text-ink3">
                    ...
                    {visibleDicts.slice(0, 2).map((d) => (
                      <span
                        key={d.id}
                        className="px-2 py-0.5 bg-parch3/50 border border-bdr/30 rounded-full"
                      >
                        {d.name}
                      </span>
                    ))}
                    <span className="opacity-60">· {sourceInfo.license}</span>
            ...
            {morphologyDisplay && (
              <div className="mb-10">
                ...
                {(() => {
                  ...
                  const displayData = morphologyDisplay.missing
                    ? aiMorphologyDisplay
                    : morphologyDisplay;
            ...
            {/* External scholarly resources — link-only dictionaries for this language. */}
            {selectedWord.lemma &&
              (() => {
                const links = dictionarySources.filter((s) => s.resolver === 'external-link');
                  ...
                  <div className="mt-4 pt-4 border-t border-bdr/30">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {links.map((src) => (
                        <a
                          key={src.id}
                          href={src.url?.(selectedWord.lemma, langId)}
                          target="_blank"
                          rel="noopener noreferrer"
        ...
        />
        {isTCExerciseOpen && <TCExerciseModal onClose={() => setIsTCExerciseOpen(false)} />}
      </AnimatePresence>
    );
  }
);
