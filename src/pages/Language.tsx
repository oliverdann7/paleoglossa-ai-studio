      ...
      >
        <div className="flex justify-between items-start mb-3">
          <div className="flex-1 pr-3">
            <h3
              className={cn('text-[20px] font-serif mb-1', rtl ? 'font-hebrew text-right' : '')}
              dir={dir}
            >
              {text.title}
            </h3>
        ...
        {text.sentenceCount != null && (
          <p className="text-[11px] text-muted mb-4">
            {t('language.sentenceCount', '{{count}} sentence available', {
              count: text.sentenceCount,
            })}
          </p>
        )}
          ...
          >
            <div className="flex flex-col md:flex-row gap-12 items-start">
              <div className="flex-1">
                ...
                <p className="text-lg text-ink2 leading-relaxed mb-6">{languageInfo.description}</p>

                <div className="flex flex-wrap gap-x-8 gap-y-4 text-sm font-sans">
                  {languageInfo.era && (
                    <div>
                      <span className="text-muted uppercase tracking-wider text-[10px] block mb-1">
                        {t('language.era', 'Era')}
                      </span>
                  ...
                  {languageInfo.writingSystem && (
                    <div>
                      <span className="text-muted uppercase tracking-wider text-[10px] block mb-1">
                        {t('language.writingSystem', 'Writing System')}
                      </span>
              ...
              {languageInfo.sampleText && (
                <div className="w-full md:w-1/3 bg-sand p-6 rounded-2xl border border-bdr flex flex-col items-center justify-center min-h-[160px]">
                  <span className="text-muted uppercase tracking-wider text-[10px] block mb-4 w-full text-center">
                    {t('language.sample', 'Sample')}
                  </span>
                  <span
                    dir={dir}
                    className={cn(
                      'text-2xl text-ink font-serif text-center leading-loose',
