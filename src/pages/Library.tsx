...
import {
  Search, Library as LibraryIcon, Play, Filter, Clock, BookOpen,
...
const PERIOD_FILTERS = [
  'Hellenistic / Roman', 'Iron Age', 'Augustan', 'Late Antique', 'Classical',
  'Archaic', 'Ancient Near Eastern', 'Classical Sanskrit', 'Late Bronze Age', 'Middle Egyptian',
];

const GENRE_FILTERS = [
  'Gospel', 'Narrative', 'Epic', 'Psalm', 'History', 'Fable',
  'Early Christian', 'Christian literature', 'Biblical prose/poetry',
  'Royal / literary text', 'Wisdom instruction',
];
...
export const Library = () => {
  ...
  const sortedTexts = useMemo(() => {
    ...
    switch (activeSort) {
      ...
      case 'newest': return copy.sort((a, b) => new Date(b.addedAt || 0).getTime() - new Date(a.addedAt || 0).getTime());
      case 'shortest': return copy.sort((a, b) => a.totalWords - b.totalWords);
      case 'unknown': return copy.sort((a, b) => {
        const aU = a.totalWords - ((a.percentKnown || 0)/100*a.totalWords) - ((a.percentLearning || 0)/100*a.totalWords);
        const bU = b.totalWords - ((b.percentKnown || 0)/100*b.totalWords) - ((b.percentLearning || 0)/100*b.totalWords);
        return bU - aU;
      });
      default: return copy;
    }
  ...
  const handleFork = async (textId: string) => {
    if (!user?.uid) return;
    setSharingId(textId);
    try { const newId = await ImportService.forkPublic(user.uid, textId); if (newId) navigate(`/app/reader/${newId}`); }
    catch (e) { console.error("Error forking:", e); }
    setSharingId(null);
  };
      ...
      <div className="mb-10 card p-6 bg-parch2/10 border-bdr/30 shadow-sm">
        <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center">
          <div className="relative flex-1 w-full max-w-sm">
            ...
            <input type="text" placeholder={t("library.searchPlaceholder", "Search texts, corpora, authors, genres...")} value={searchQuery}
          ...
          <div className="flex flex-wrap gap-2 flex-1">
            {mainFilters.slice(0, 5).map(f => (
              <button key={f.id} onClick={() => setActiveLang(f.name)}
                className={cn("px-4 py-2 rounded-full text-[12px] font-medium font-sans transition-all duration-150 border flex items-center gap-1.5 active:scale-95",
                  activeLang === f.name ? "bg-blue text-white shadow-md border-blue" : "bg-white text-ink3 border-bdr/60 hover:bg-parch hover:border-blue/30")}>
                <span>{f.icon}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
          {[1,2,3,4,5,6].map(i => (
            <div key={i} className="card p-6 min-h-[240px] animate-pulse">
              ...
              <div className="flex gap-1.5 mb-4"><div className="h-5 bg-parch3 rounded-full w-16" /><div className="h-5 bg-parch3 rounded-full w-20" /></div>
              <div className="h-2 bg-parch3 rounded w-full mb-2" />
              <div className="flex justify-between"><div className="h-4 bg-parch3 rounded w-24" /><div className="h-6 bg-parch3 rounded-full w-8" /></div>
            </div>
          ))}
        </div>
          ...
          {Object.entries(collections).map(([name, colTexts]) => (
            <div key={name} className="scroll-mt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {colTexts.map((text, i) => (
                  <motion.div key={text.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03, duration: 0.3 }}
                    onClick={() => navigate(`/app/reader/${text.id}`)}
                    className={cn("card p-6 flex flex-col justify-between cursor-pointer group hover:border-blue/30 transition-all min-h-[250px] relative overflow-hidden",
                      text.sourceType === 'import' && "border-blue/10 bg-blue/[0.01]")}>
                    <div>
                      <h4 className="text-[18px] font-serif font-medium text-ink leading-snug mb-1">{text.title}</h4>
                      <div className="flex items-center gap-2 mb-4">
                        <span className="text-[10px] text-ink3 uppercase tracking-[0.1em] font-bold">{text.author}</span>
                      ...
                      <div className="flex h-1 gap-0.5 mb-6 opacity-40 group-hover:opacity-100 transition-opacity">
                        ...
                        <div className="bg-amber h-full" style={{ width: `${Math.max(text.percentLearning || 0, 1)}%` }} />
                    ...
                    <div className="flex items-end justify-between pt-4 mt-auto border-t border-dashed border-bdr/40">
                      <div className="flex items-center gap-2">
                        <span className={cn("text-[10px] uppercase font-bold tracking-widest flex items-center gap-1",
                      ...
                      <span className={cn("pill text-[10px] px-2 py-0.5", getCefrClass(text.level))}>{text.level}</span>
                    </div>
                  </motion.div>
                ))}
              </div>
