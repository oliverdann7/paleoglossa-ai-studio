...
export const Library = () => {
  ...
  const allTexts = useMemo(() => {
    ...
    const unified = [...builtIn, ...imported.map((t: any) => ({
       ...t,
       isImported: true,
       author: 'Your Import',
      ...
      <div className="flex flex-col lg:flex-row gap-8 mb-10 items-start lg:items-end">
        ...
        <div className="w-full lg:w-64 card p-4 bg-parch2/20 border-bdr/30">
           <div className="flex justify-between items-center mb-4">
              ...
              <span className="text-[11px] font-bold text-blue">{minKnown}%+ Known</span>
           </div>
           <input 
              type="range"
              min="0"
          ...
          {filteredTexts.map((text, i) => (
           ...
           >
             <div>
               <div className="flex justify-between items-start mb-2">
                 <h4 className="text-[18px] font-serif font-medium text-ink leading-snug pr-4">
                    {text.title}
                    {text.isImported && <span className="inline-block ml-2 text-[10px] font-sans font-bold bg-blue/10 text-blue px-2 py-0.5 rounded uppercase tracking-wider">📥 Import</span>}
                 </h4>
               ...
               <div className="flex h-1 gap-0.5 mb-6 opacity-30 group-hover:opacity-100 transition-opacity">
                  <div className="bg-blue h-full" style={{ width: `${text.percentKnown}%` }} />
                  <div className="bg-amber h-full" style={{ width: `${text.percentLearning}%` }} />
             ...
             <div className="flex items-end justify-between pt-4 mt-auto">
                <span className="text-[11px] font-body italic text-muted">{text.isImported ? "Private Lesson" : "Original Scripture"}</span>
