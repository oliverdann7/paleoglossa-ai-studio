...
import {
  ...
  Tag, Loader2, FileText, Clock, Search, Download, Printer,
  Eye, Code2,
} from "lucide-react";
...
function parseWikilinks(content: string): string {
  return content
    .replace(/\[\[text:([^\]|]+)\|?([^\]]*)\]\]/g, (_, id, label) =>
      `[${label || id}](/app/reader/${id})`
    )
    .replace(/\[\[([^\]|:]+)\|?([^\]]*)\]\]/g, (_, lemma, label) =>
      `[${label || lemma}](/app/dictionary/grc/${encodeURIComponent(lemma)})`
    );
}
...
function WikilinkAnchor({ href, children }: { href?: string; children?: React.ReactNode }) {
  ...
  return <a href={href} target="_blank" rel="noopener noreferrer" className="text-blue underline">{children}</a>;
}
...
function NoteCard({ note, isSelected, onClick, onEdit, onDelete }: {
  ...
  const relativeTime = note.updatedAt
    ...
    : note.createdAt ? formatDistanceToNow(new Date(note.createdAt), { addSuffix: true }) : null;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-[13px] font-medium text-ink line-clamp-2 flex-1">{preview || "Empty note"}</p>
        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={e => { e.stopPropagation(); onEdit(note); }} className="p-1 text-muted hover:text-blue rounded"><Pencil className="w-3.5 h-3.5" /></button>
          <button onClick={e => { e.stopPropagation(); onDelete(note.id); }} className="p-1 text-muted hover:text-red-500 rounded"><Trash2 className="w-3.5 h-3.5" /></button>
        </div>
      </div>
      {note.lemma && <span className="text-[11px] font-mono text-blue mt-1 block">{note.lemma}</span>}
      {note.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {note.tags.map(tag => (
            <span key={tag} className="text-[9px] font-bold uppercase tracking-wider bg-parch3 text-muted px-1.5 py-0.5 rounded">{tag}</span>
          ))}
        </div>
      )}
...
function NoteEditor({ initial, onSave, onCancel, isSaving }: {
  initial?: Note | null;
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2 border-b border-bdr/30 shrink-0">
        ...
        <button
          ...
          className={cn("flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-colors",
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-2 border-b border-bdr/30 shrink-0">
        <div className="flex items-center gap-2">
          {note.lemma && <span className="font-mono text-[12px] text-blue font-bold">{note.lemma}</span>}
          {relativeTime && <span className="text-[10px] text-muted">{relativeTime}</span>}
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={exportMd} title="Download as Markdown" className="p-1.5 text-muted hover:text-ink rounded transition-colors">
            <Download className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => window.print()} title="Print / Save as PDF" className="p-1.5 text-muted hover:text-ink rounded transition-colors">
            <Printer className="w-3.5 h-3.5" />
          </button>
          <button onClick={onEdit} className="flex items-center gap-1 px-2 py-1 bg-blue/10 text-blue rounded text-[10px] font-bold uppercase tracking-widest hover:bg-blue/20 transition-colors">
      ...
      {note.tags.length > 0 && (
        <div className="border-t border-bdr/20 px-4 py-2 flex flex-wrap gap-1.5 shrink-0">
          {note.tags.map(tag => (
            <span key={tag} className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider bg-parch3 text-muted px-2 py-0.5 rounded-full border border-bdr/30">
...
export const NotebookDetail = () => {
  ...
  useEffect(() => {
    ...
    apiFetch<Note[]>(`/api/notes?notebookId=${notebookId}`)
      .then(data => setNotes([...data].sort((a, b) => (b.updatedAt ?? b.createdAt ?? "").localeCompare(a.updatedAt ?? a.createdAt ?? ""))))
      .catch(() => setNotes([]))
  ...
  const startNewNote = () => { setIsCreating(true); setEditingNote(null); setSelectedNote(null); };
  const showEditor = isCreating || editingNote !== null;
  const rightPanelNote = editingNote ?? selectedNote;

  if (isLoadingNotebook) {
    return <div className="p-12 text-center"><Loader2 className="w-6 h-6 animate-spin text-blue mx-auto" /></div>;
  }
    ...
    <div className="flex flex-col h-screen font-sans">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-bdr/40 bg-parch/80 backdrop-blur-sm shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/app/notebooks")} className="flex items-center gap-1 text-muted hover:text-ink text-[12px] font-medium transition-colors">
        ...
        <div className="flex items-center gap-2">
          <button onClick={exportAllMd} title="Export as Markdown" className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest text-muted hover:text-ink border border-bdr/40 rounded-lg hover:bg-parch3 transition-colors">
            <Download className="w-3.5 h-3.5" /> Export
          </button>
          <button onClick={startNewNote} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue text-white font-bold rounded-lg text-[11px] uppercase tracking-widest hover:opacity-90 transition-opacity">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 shrink-0 border-r border-bdr/40 flex flex-col bg-parch/40 overflow-hidden">
              ...
              {allTags.map(tag => (
                <button
                  ...
                  className={cn(
                    ...
                    activeTag === tag ? "bg-blue text-white border-blue" : "bg-parch3 text-muted border-bdr/30 hover:border-blue/40",
                  )}
                >
                  {tag}
                </button>
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoadingNotes ? (
              <div className="p-8 text-center"><Loader2 className="w-5 h-5 animate-spin text-blue mx-auto" /></div>
            ) : filteredNotes.length === 0 ? (
              <div className="p-8 text-center text-muted">
                <FileText className="w-8 h-8 mx-auto mb-2 opacity-40" />
                <p className="text-[12px]">{searchQuery || activeTag ? "No matching notes" : "No notes yet"}</p>
              </div>
            ) : (
              filteredNotes.map(note => (
                <NoteCard
                  ...
                  onClick={() => { setSelectedNote(note); setEditingNote(null); setIsCreating(false); }}
                  onEdit={n => { setEditingNote(n); setIsCreating(false); setSelectedNote(null); }}
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-white/40">
          {showEditor ? (
            ...
            <NoteViewer
              note={rightPanelNote}
              onEdit={() => { setEditingNote(rightPanelNote); setSelectedNote(null); }}
            />
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-muted gap-3">
              ...
              <button onClick={startNewNote} className="text-[12px] text-blue font-bold underline hover:no-underline">New Note</button>
            </div>
          )}
        </div>
      </div>
