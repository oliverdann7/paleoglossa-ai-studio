...
export const Vocabulary = () => {
  ...
  const getDictionaryPath = (lemma: string, langId: string) => {
    ...
    const languageId = code.includes('koine')
      ? 'grc-koine'
      : code.includes('greek')
        ? 'grc'
        : code.includes('hebrew')
          ? 'hbo'
          : code.includes('latin')
            ? 'lat'
            : langId;
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      ...
      case 'Learning':
        return 'cefr-a';
      case 'Seen':
        return 'cefr-a opacity-50';
      case 'Ignored':
        return 'bg-ink3/10 text-ink3 border-ink3/20';
      default:
        return '';
    }
  };
      ...
      <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="font-body text-[15px] italic text-ink2">
            {t(
              'vocab.personalCollection',
              `Your personal collection of ${words.length} tracked words`,
              { count: words.length }
            )}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() =>
              exportToCSV(
                filteredWords,
                `vocabulary-${activeLanguageId}-${new Date().toISOString().slice(0, 10)}.csv`
              )
            }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="card p-5 border-blue/20 bg-blue/5">
          <div className="flex items-center gap-3 mb-2 opacity-70">
            ...
            <h4 className="text-[12px] uppercase tracking-widest font-bold text-blue">
              {t('vocab.reviewsDue', 'Reviews Due')}
            </h4>
          </div>
        ...
        <div className="card p-5 border-emerald-500/20 bg-emerald-500/5">
          <div className="flex items-center gap-3 mb-2 opacity-70">
            ...
            <h4 className="text-[12px] uppercase tracking-widest font-bold text-emerald-600">
        ...
        <div className="card p-5 border-amber/20 bg-amber/5">
          <div className="flex items-center gap-3 mb-2 opacity-70">
            ...
            <h4 className="text-[12px] uppercase tracking-widest font-bold text-amber">
              {t('vocab.learning', 'Words Learning')}
            </h4>
          </div>
            ...
            >
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    {word.status !== 'Seen' && (
                      <span
                        className={cn(
                          'text-[10px] font-bold flex items-center gap-1.5 uppercase tracking-wider',
                      ...
                      >
                        ...
                        {word.isDue
                          ...
                          : t('vocab.dueDate', {
                              date: new Date(word.nextReview).toLocaleDateString(),
                              defaultValue: `Due ${new Date(word.nextReview).toLocaleDateString()}`,
                            })}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto mt-2 md:mt-0 pt-3 md:pt-0 border-t border-bdr/50 md:border-none">
                <div className="flex flex-col items-start md:items-end gap-1 w-36">
                  <div
                    className={cn(
                      'pill px-2 py-0.5 text-[10.5px] font-bold shadow-sm',
                      getStatusClass(word.status)
                    )}
                  >
                    ...
                    <div className="text-[10px] text-muted font-mono">
                      ...
                      {word.lastSeenAt && (
                        <span className="ml-1 opacity-70">
                          · {new Date(word.lastSeenAt).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  )}
                ...
                <div className="flex items-center gap-1.5 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                  <button
                    onClick={() => navigate(getDictionaryPath(word.term, word.language))}
                    className="p-1.5 rounded text-muted hover:text-blue hover:bg-blue/5 transition-colors"
                    title={t('vocab.openDictionary', 'Open dictionary entry')}
                  >
          ...
          <div className="flex items-center justify-center gap-4 py-8">
            ...
            <span className="text-[13px] font-bold text-muted">
              ...
              <span className="font-normal ml-2">
                {t('vocab.totalWords', '({{count}} words)', { count: filteredWords.length })}
              </span>
            </span>
            <button
