...
// ─── Token Lookup ─────────────────────────────────────────────────────────────
app.get('/api/tokens/:language/:token', (req: any, res: any) => {
  try {
...
const PRICE_IDS: Record<string, { monthly?: string; yearly?: string }> = {
  basic_1: {
    monthly: process.env.STRIPE_BASIC_PRICE_ID || 'REDACTED_STRIPE_PRICE_ID',
    yearly: process.env.STRIPE_BASIC_YEARLY_PRICE_ID || 'REDACTED_STRIPE_PRICE_ID',
  },
  duo_2: {
    monthly: process.env.STRIPE_DUO_PRICE_ID || 'REDACTED_STRIPE_PRICE_ID',
    yearly: process.env.STRIPE_DUO_YEARLY_PRICE_ID || 'REDACTED_STRIPE_PRICE_ID',
  },
  full_all: {
    monthly: process.env.STRIPE_FULL_PRICE_ID || 'REDACTED_STRIPE_PRICE_ID',
    yearly: process.env.STRIPE_FULL_YEARLY_PRICE_ID || 'REDACTED_STRIPE_PRICE_ID',
  },
};
...
app.post('/api/admin/refresh-claims', requireAuth as any, async (req: AuthenticatedRequest, res: any) => {
  ...
  if (isAdminEmail) {
    ...
    // Ensure admin accounts have full plan access in Firestore
    const adminDb_ = getAdminDb();
    if (adminDb_) {
...
app.get('/api/admin/users', requireAuth as any, async (req: AuthenticatedRequest, res: any) => {
  if (!requireAdmin(req, res)) return;
  const adminDb_ = getAdminDb();
  const auth_ = getAdminAuth();
  ...
  try {
    ...
    const rows: any[] = [];
    // Batch fetch Auth records (max 100 per call)
...
app.post('/api/admin/users/:uid/set-plan', requireAuth as any, async (req: AuthenticatedRequest, res: any) => {
  if (!requireAdmin(req, res)) return;
  const adminDb_ = getAdminDb();
...
// Grant or revoke admin claim
app.post('/api/admin/users/:uid/set-admin', requireAuth as any, async (req: AuthenticatedRequest, res: any) => {
  if (!requireAdmin(req, res)) return;
  const auth_ = getAdminAuth();
  const adminDb_ = getAdminDb();
